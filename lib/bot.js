	...
	this.post = function(text, replyToID, callback) {
		...
		this.oauth.post(
			...
			function(err, response) {
				if (err) cb(err); else cb(err, response);
			}
		);
	}

	//Pull a list of the account's mentions from the database
	this.mentions = function(amount, callback) {
		var amt = parseInt(amount) || 20;
		var cb = callback || function(err, data) {console.log(err || '[MENTIONS] retrieved!');};
		
		this.oauth.get(
			'http://api.twitter.com/1/statuses/mentions.json?count='+amt,
			this.options.token,
			this.options.token_secret,
			function(err, response) {
				if (err) cb(err); else cb(err, response);
